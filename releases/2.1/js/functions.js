...
function setPuyoStyle()
{
	if (style == STYLE_EYE_CANDY)
	{
		...
		puyoNuisanceIndicator = 
			DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/nuisance_indicator.png';
		
		/* Load images for X Display */
		puyoXDisplay = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/x_display.png';
	}
	else
	{
	...
	puyo[PUYO.NONE]     = '';
	puyo[PUYO.DELETE]   = DIRECTORY_IMAGES + '/' + (puyoSize == PUYO.SIZE.SMALL ? 'delete_small.png' : 'delete.png');
	puyo[PUYO.RED]      = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_R.png';
	puyo[PUYO.GREEN]    = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_G.png';
	puyo[PUYO.BLUE]     = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_B.png';
	puyo[PUYO.YELLOW]   = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_Y.png';
	puyo[PUYO.PURPLE]   = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_P.png';
	puyo[PUYO.NUISANCE] = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_N.png';
	puyo[PUYO.POINT]    = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_T.png';
	puyo[PUYO.HARD]     = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_H.png';
	puyo[PUYO.IRON]     = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_I.png';
	puyo[PUYO.WALL]     = DIRECTORY_IMAGES + '/puyo/' + puyoDir + '/puyo_W.png';
...
function doChain()
{
	...
	if (chainMade)
	{
		...
		if (playMode)
			timerFall = setTimeout('puyoFall()', curPopSpeed);
	}
...
function puyoFall()
{
	...
	if (playMode)
		timerChain = setTimeout('doChain()', curPopSpeed);
}
...
** Fever Chains
*****************************************/
// The initalization has been moved to feverchains.js.

// Display the fever chain
function displayFeverChain(game, set, chain)
{
	var feverChain = FeverChains[game].sets[set].chains[curColors + 3][chain];

	if (!editMode)
		editChain();
	
	/* Make sure the fever chain isn't undefined */
	if (typeof feverChain == 'undefined')
		feverChain = '';
	
	// Change the board with and height if it is not the same size as the fever chain
	if (FeverChains[game].field.width != fieldWidth || FeverChains[game].field.height != fieldHeight - 1)
	{
		fieldWidth  = FeverChains[game].field.width;
		fieldHeight = FeverChains[game].field.height + 1;
...
function loadOptions()
{
	...
	getObject('widget_options_main_content').innerHTML = loadHTML(URL.XML.options);
...
function generateLinkingCodes()
{
	...
	{
		// If the field size isn't normal and the field only allows normal ones, empty it
		if (linkingCodes[i].standardOnly && (fieldWidth != 6 || fieldHeight != 13))
			getObject(linkingCodes[i].id).value = '';
		else if (!linkingCodes[i].standardOnly && (fieldWidth != 6 || fieldHeight != 13))
			getObject(linkingCodes[i].id).value = linkingCodes[i].start + '(' + fieldWidth + ',' + (fieldHeight - 1) + ')' + chainID + linkingCodes[i].end;
		else
			getObject(linkingCodes[i].id).value = linkingCodes[i].start + chainID + linkingCodes[i].end;
	}
}
...
function setCharacterPower(value)
{
	...
	for (var i = 0; i < cp_start.length - 1; i++)
	{
		...
		{
			// This is the value, set the game mode, current character power, and update the label
			gameMode = (CharacterPowers[i].mode == MODE.CLASSIC ? MODE.CLASSIC : MODE.FEVER);
...
{
	// Load the XMLHttpRequest object, or one of IE's variants if it doesn't exist.
	...
	// Set up the request, then load and return the XML
