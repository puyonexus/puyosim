...
var Field = Class.extend({
	...
	setChain: function(width, height, chain) { // Set the chain used for the current field
		...
		width  = Math.min(Math.max(width, Field.MinWidth), Field.MaxWidth);
		height = Math.min(Math.max(height, Field.MinHeight), Field.MaxHeight);
	...
	parseChainQuery: function() { // Parses the chain query string (location.search)
		...
		if (location.search.length > 2 && chainQuery.charAt(1) == '?') // Old Style Chain Query (v1 & v2)
		{
			...
			if (query.search(sizeExp) == 0) // String contains chain width & height
			{
				...
				this.setChain(parseInt(size[0], 10) || Field.DefaultWidth, parseInt(size[1], 10) || Field.DefaultHeight, query.substring(match.length));
			}
		...
		else // New Style Chain Query (v3)
		{
			...
			if (query.chain) // If a chain exists in the query, use it
				this.setChain(parseInt(query.w, 10) || Field.DefaultWidth, parseInt(query.h, 10) || Field.DefaultHeight, query.chain);
		}
	},
