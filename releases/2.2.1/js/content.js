...
function displayPuyoField()
{
	...
	for (var y = 0; y < fieldHeight; y++)
	{
		...
		content += '<div class="clear"></div>'; // go to the next row
		
		// Set up the background after
		if (y == 0)
		{
...
function displayLinkingCodes()
{
	...
	for (var i = 0; i < linkingCodes.length; i++)
	{
		content += 
			...
			'<span class="small">(<a href="javascript: void(0);" onclick="selectText(\'' + linkingCodes[i].id + '\');">Select</a>)</span>' +
			(linkingCodes[i].shortenUrl && location.protocol != 'file:' ? '&nbsp;<span class="small">(<input type="checkbox" id="linkingcode-shortenurl"><label for="linkingcode-shortenurl">&nbsp;Shorten Url</label>)</span>' : '') +
			'<br />' +
...
function displayPuyoSkins_small()
{
	...
	for (i in puyoSkins_small)
	{
		...
		for (j = 0; j < puyo_colors.length; j++)
			content += '<span class="puyo_small" style="margin-left: 4px; background-image: url(\'' + DIRECTORY.IMAGES + '/puyo/' + puyoSkins_small[i] + '/puyo_' + puyo_colors[j] + '.png\');"></span>';
...
function displayPuyoSkins_large()
{
	...
	for (i in puyoSkins_large)
	{
		...
		/* Display each puyo the fun way ;) */
		for (j = 0; j < puyo_colors.length; j++)
			content += '<span class="puyo_large" style="margin-left: 4px;  background-image: url(\'' + DIRECTORY.IMAGES + '/puyo/' + puyoSkins_large[i] + '/puyo_' + puyo_colors[j] + '.png\');"></span>';
