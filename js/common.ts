function pnNavbarToggleOnClick(event: Event) {
    ...
    const navbarCollapse = target.parentElement?.getElementsByClassName("pn-navbar-collapse").item(0);
...
function init() {
    ...
    const pnNavbarCollapses = document.getElementsByClassName("pn-navbar-collapse");
    ...
    for (var i = 0; i < pnNavbarCollapses.length; i++) {
        pnNavbarCollapses[i].addEventListener("ontransitionend", pnNavbarCollapseOnTransitionEnd);
    }
};
