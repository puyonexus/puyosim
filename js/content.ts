...
interface ISimpleFieldType {
  CSSClass: "field-basic"|"field-standard";
}

interface IEyeCandyFieldType {
...
export type IFieldType = ISimpleFieldType | IEyeCandyFieldType;

interface IContent {
  Field: {
    Basic: ISimpleFieldType,
    Standard: ISimpleFieldType,
    EyeCandy: IEyeCandyFieldType
  }
}

const eyeCandy: IEyeCandyFieldType = {
  CSSClass: "field-eyecandy",
  ...
  Script: () => {
    $("#field-bg-1").css(
      ...
      eyeCandy.StageBGs[
      Math.floor(
        Math.random() * eyeCandy.StageBGs.length
      )
      ] +
      "')"
    );
    ...
    if (boardBackgroundId === 0) {
      $("#field-bg-2").css(
        ...
        eyeCandy.CharaBGs[
        Math.floor(
          Math.random() * eyeCandy.CharaBGs.length
        )
        ] +
        "')"
      );
      ...
      $("#field-bg-2").css(
        "background-image",
        "url('/images/eyecandy/field_char_bg/" +
        eyeCandy.CharaBGs[boardBackgroundId - 1] +
        "')"
      );
    }
  },
...
export const content: IContent = {
  Field: {
    Basic: { CSSClass: "field-basic" },
    Standard: { CSSClass: "field-standard" },
    EyeCandy: eyeCandy,
  },
};
